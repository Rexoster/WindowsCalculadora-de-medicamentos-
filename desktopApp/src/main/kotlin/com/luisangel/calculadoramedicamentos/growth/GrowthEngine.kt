...
class GrowthEngine {
    ...
    private fun loadAgeTables(): Map<Pair<GrowthSex, GrowthIndicator>, List<LmsPoint>> {
        ...
        openResource("who_age_lms.csv").bufferedReader().useLines { lines ->
    ...
    private fun loadWeightHeightTables(): Map<Pair<GrowthSex, MeasurementMode>, List<LmsPoint>> {
        ...
        openResource("who_weight_height_lms.csv").bufferedReader().useLines { lines ->
            lines.drop(1).forEach { line ->
                val values = line.split(',')
                if (values.size < 6) return@forEach
                val point = LmsPoint(
    ...
    private fun openResource(name: String): java.io.InputStream =
        Thread.currentThread().contextClassLoader.getResourceAsStream(name)
            ?: GrowthEngine::class.java.classLoader.getResourceAsStream(name)
            ?: error("No se encontró el recurso $name en la aplicación de Windows.")
